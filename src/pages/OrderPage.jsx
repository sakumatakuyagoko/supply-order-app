...
function OrderPage() {
    ...
    const handleConfirmOrder = async (ordersPayload) => {
        if (!currentEmployee) return;

        setIsSubmitting(true);
        try {
            const dataToSubmit = ordersPayload || items;
            const result = await submitOrder(dataToSubmit, currentEmployee);
            ...
            <main className="flex-grow flex flex-col h-full overflow-hidden w-full">
                <header className="bg-blue-600 shadow-sm z-10 p-2 md:p-4 flex justify-between items-center h-16 md:h-20 transition-all">
                    <div className="flex items-center gap-4 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => navigate('/')}>
                        ...
                        <svg className="hidden md:block w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
